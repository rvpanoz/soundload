...
class SnapTarget extends (_core || _load_core()).Target {
    ...
    build(appOutDir, arch) {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            (0, (_electronBuilderUtil || _load_electronBuilderUtil()).log)(`Building Snap for arch ${(_electronBuilderUtil || _load_electronBuilderUtil()).Arch[arch]}`);
            ...
            if (process.platform === "darwin" && process.env.USE_SYSTEM_SNAPCAFT) {
                try {
                    ...
                    const env = Object.assign({}, process.env, { LC_ALL: "en_US.UTF-8", LANG: "en_US.UTF-8" });
                    delete env.VERSIONER_PYTHON_VERSION;
                    delete env.VERSIONER_PYTHON_PREFER_32_BIT;
                    yield (0, (_electronBuilderUtil || _load_electronBuilderUtil()).exec)("snapcraft", ["--version"], {
                        // execution because Python 3 was configured to use ASCII as encoding for the environment
                        env
                    });
                    isUseDocker = false;
                } catch (e) {
                    (0, (_electronBuilderUtil || _load_electronBuilderUtil()).debug)(`snapcraft not installed: ${e}`);
                }
            }
            ...
            const snapFileName = `${snap.name}_${snap.version}_${(0, (_electronBuilderUtil || _load_electronBuilderUtil()).toLinuxArchString)(arch)}.snap`;
            ...
            if (isUseDocker) {
                ...
                commands.push("cd /s");
                commands.push(`snapcraft snap --target-arch ${(0, (_electronBuilderUtil || _load_electronBuilderUtil()).toLinuxArchString)(arch)} -o /out/${snapFileName}`);
                ...
                "-v", `${_this.outDir}:/out`, "electronuserland/electron-builder:latest", "/bin/bash", "-c", commands.join(" && ")], {
                ...
                yield (0, (_electronBuilderUtil || _load_electronBuilderUtil()).spawn)("snapcraft", ["snap", "--target-arch", (0, (_electronBuilderUtil || _load_electronBuilderUtil()).toLinuxArchString)(arch), "-o", resultFile], {
                    cwd: stageDir,
