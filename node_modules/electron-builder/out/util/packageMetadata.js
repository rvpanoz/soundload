...
const normalizeData = require("normalize-package-data");function checkMetadata(metadata, devMetadata, appPackageFile, devAppPackageFile) {
    ...
    if ((0, (_electronBuilderUtil || _load_electronBuilderUtil()).isEmptyOrSpaces)(metadata.description)) {
        (0, (_electronBuilderUtil || _load_electronBuilderUtil()).warn)(`description is missed in the package.json (${appPackageFile})`);
    }
    if (!metadata.author) {
        (0, (_electronBuilderUtil || _load_electronBuilderUtil()).warn)(`author is missed in the package.json (${appPackageFile})`);
    }
    ...
    if (devDependencies != null && "electron-rebuild" in devDependencies) {
        (0, (_electronBuilderUtil || _load_electronBuilderUtil()).log)('electron-rebuild not required if you use electron-builder, please consider to remove excess dependency from devDependencies\n\nTo ensure your native dependencies are always matched electron version, simply add script `"postinstall": "electron-builder install-app-deps" to your `package.json`');
    }
    if (errors.length > 0) {
