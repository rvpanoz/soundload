...
let createCustomCertKeychain = (() => {
    var _ref2 = (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
        ...
        const tmpKeychainPath = _path.join((0, (_electronBuilderUtil || _load_electronBuilderUtil()).getCacheDirectory)(), (0, (_tempFile || _load_tempFile()).getTempName)("electron-builder-root-certs"));
...
let removeKeychain = (() => {
    var _ref3 = (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* (keychainFile) {
        try {
            yield (0, (_electronBuilderUtil || _load_electronBuilderUtil()).exec)("security", ["delete-keychain", keychainFile]);
        } catch (e) {
    ...
    return function removeKeychain(_x4) {
        return _ref3.apply(this, arguments);
    };
})();

let createKeychain = exports.createKeychain = (() => {
    var _ref4 = (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* (_ref5) {
        let tmpDir = _ref5.tmpDir,
            cscLink = _ref5.cscLink,
            cscKeyPassword = _ref5.cscKeyPassword,
            cscILink = _ref5.cscILink,
            cscIKeyPassword = _ref5.cscIKeyPassword,
            currentDir = _ref5.currentDir;

        // travis has correct AppleWWDRCA cert
        if (process.env.TRAVIS !== "true") {
            yield bundledCertKeychainAdded.value;
        }
        const keychainFile = yield tmpDir.getTempFile(".keychain", false, removeKeychain);
        ...
        }), (_bluebirdLst2 || _load_bluebirdLst2()).default.mapSeries([["create-keychain", "-p", keychainPassword, keychainFile], ["unlock-keychain", "-p", keychainPassword, keychainFile], ["set-keychain-settings", keychainFile]], function (it) {
        ...
        if (!(list.indexOf(keychainFile) !== -1)) {
            yield (0, (_electronBuilderUtil || _load_electronBuilderUtil()).exec)("security", ["list-keychains", "-d", "user", "-s", keychainFile].concat(list));
        }
    ...
    return function createKeychain(_x5) {
...
let importCerts = (() => {
    ...
    return function importCerts(_x6, _x7, _x8) {
...
let getValidIdentities = (() => {
    ...
    return function getValidIdentities(_x9) {
...
let _findIdentity = (() => {
    ...
    return function _findIdentity(_x10, _x11, _x12) {
...
function listUserKeychains() {
    return (0, (_electronBuilderUtil || _load_electronBuilderUtil()).exec)("security", ["list-keychains", "-d", "user"]).then(it => it.split("\n").map(it => {
        const r = it.trim();
