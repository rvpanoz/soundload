...
class PlatformPackager {
    ...
    getCscPassword() {
        ...
        if ((0, (_electronBuilderUtil || _load_electronBuilderUtil()).isEmptyOrSpaces)(password)) {
            (0, (_electronBuilderUtil || _load_electronBuilderUtil()).log)("CSC_KEY_PASSWORD is not defined, empty password will be used");
            return "";
        } else {
    ...
    computeAppOutDir(outDir, arch) {
        return this.packagerOptions.prepackaged || _path.join(outDir, `${this.platform.buildConfigurationKey}${(0, (_electronBuilderUtil || _load_electronBuilderUtil()).getArchSuffix)(arch)}${this.platform === (_core || _load_core()).Platform.MAC ? "" : "-unpacked"}`);
    }
    ...
    doPack(outDir, appOutDir, platformName, arch, platformSpecificBuildOptions, targets) {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            const macroExpander = function (it) {
                return _this2.expandMacro(it, arch == null ? null : (_electronBuilderUtil || _load_electronBuilderUtil()).Arch[arch], { "/*": "{,/**/*}" });
            };
            ...
            (0, (_electronBuilderUtil || _load_electronBuilderUtil()).log)(`Packaging for ${platformName} ${(_electronBuilderUtil || _load_electronBuilderUtil()).Arch[arch]} using ${isElectron ? `electron ${config.electronVersion}` : `muon ${muonVersion}`} to ${_path.relative(_this2.projectDir, appOutDir)}`);
            if (_this2.info.isPrepackedAppAsar) {
                yield (0, (_dirPackager || _load_dirPackager()).unpackElectron)(_this2, appOutDir, platformName, (_electronBuilderUtil || _load_electronBuilderUtil()).Arch[arch], config.electronVersion);
            } else {
                yield isElectron ? (0, (_dirPackager || _load_dirPackager()).unpackElectron)(_this2, appOutDir, platformName, (_electronBuilderUtil || _load_electronBuilderUtil()).Arch[arch], config.electronVersion) : (0, (_dirPackager || _load_dirPackager()).unpackMuon)(_this2, appOutDir, platformName, (_electronBuilderUtil || _load_electronBuilderUtil()).Arch[arch], muonVersion);
            }
            ...
            const taskManager = new (_asyncTaskManager || _load_asyncTaskManager()).AsyncTaskManager(_this2.info.cancellationToken);
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            if (result === false) {
                ...
                if (appAsarStat == null || !appAsarStat.isFile()) {
                    (0, (_electronBuilderUtil || _load_electronBuilderUtil()).warn)("Packaging using asar archive is disabled — it is strongly not recommended.\n" + "Please enable asar and use asarUnpack to unpack files that must be externally available.");
                }
                return null;
            }
    ...
    computeSafeArtifactName(ext, arch) {
        ...
        return this.computeArtifactName("${name}-${version}-${arch}.${ext}", ext, skipArchIfX64 && arch === (_electronBuilderUtil || _load_electronBuilderUtil()).Arch.x64 ? null : arch);
    }
    expandArtifactNamePattern(targetSpecificOptions, ext, arch, defaultPattern) {
        ...
        return this.computeArtifactName(pattern, ext, skipArchIfX64 && arch === (_electronBuilderUtil || _load_electronBuilderUtil()).Arch.x64 ? null : arch);
    }
    computeArtifactName(pattern, ext, arch) {
        let archName = arch == null ? null : (_electronBuilderUtil || _load_electronBuilderUtil()).Arch[arch];
        if (arch === (_electronBuilderUtil || _load_electronBuilderUtil()).Arch.x64) {
            if (ext === "AppImage" || ext === "rpm") {
                archName = "x86_64";
            } else if (ext === "deb") {
                archName = "amd64";
            }
        } else if (arch === (_electronBuilderUtil || _load_electronBuilderUtil()).Arch.ia32) {
    ...
    generateName(ext, arch, deployment) {
        ...
        } else if (arch === (_electronBuilderUtil || _load_electronBuilderUtil()).Arch.ia32 && ext === "deb") {
            c = "i386";
        } else if (ext === "pacman") {
            if (arch === (_electronBuilderUtil || _load_electronBuilderUtil()).Arch.ia32) {
                c = "i686";
            }
            e = "pkg.tar.xz";
        } else {
            c = (_electronBuilderUtil || _load_electronBuilderUtil()).Arch[arch];
        }
        if (c == null) {
            c = classifier;
    ...
    getDefaultIcon(ext) {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            if (resourceList.indexOf(name) !== -1) {
                ...
                (0, (_electronBuilderUtil || _load_electronBuilderUtil()).warn)("Application icon is not set, default Electron icon will be used");
                return null;
            }
        })();
    }
