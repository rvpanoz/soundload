...
function getMainFileMatchers(appDir, destination, macroExpander, platformSpecificBuildOptions, packager, outDir, isElectronCompile) {
    ...
    patterns.push("!**/{.git,.hg,.svn,CVS,RCS,SCCS," + "__pycache__,.DS_Store,thumbs.db,.gitignore,.gitkeep,.gitattributes,.npmignore," + ".idea,.vs,.editorconfig,.flowconfig,.jshintrc,.eslintrc," + ".yarn-integrity,.yarn-metadata.json,yarn-error.log,yarn.lock,package-lock.json,npm-debug.log," + "appveyor.yml,.travis.yml,circle.yml,.nyc_output}");
    if (isElectronCompile) {
        patterns.push("!.cache{,/**/*}");
    }
    (0, (_electronBuilderUtil || _load_electronBuilderUtil()).debug)(`File patterns of first/default matcher:\n\t${patterns.join("\n\t")}`);
    return matchers;
}
/** @internal */
...
function copyFiles(matchers) {
    ...
    return (_bluebirdLst2 || _load_bluebirdLst2()).default.map(matchers, (() => {
        var _ref = (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* (matcher) {
            ...
            if (fromStat == null) {
                (0, (_electronBuilderUtil || _load_electronBuilderUtil()).warn)(`File source ${matcher.from} doesn't exist`);
                return;
            }
            if (fromStat.isFile()) {
            ...
            if ((_electronBuilderUtil || _load_electronBuilderUtil()).debug.enabled) {
                (0, (_electronBuilderUtil || _load_electronBuilderUtil()).debug)(`Copying files using pattern: ${matcher}`);
            }
