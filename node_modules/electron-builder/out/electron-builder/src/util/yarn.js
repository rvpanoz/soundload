...
let installOrRebuild = exports.installOrRebuild = (() => {
    var _ref = (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* (config, appDir, options) {
        let forceInstall = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;

        const effectiveOptions = Object.assign({ buildFromSource: config.buildDependenciesFromSource === true, additionalArgs: (0, (_electronBuilderUtil || _load_electronBuilderUtil()).asArray)(config.npmArgs) }, options);
...
let rebuild = exports.rebuild = (() => {
    var _ref2 = (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* (appDir, options) {
        ...
        if (nativeDeps.length === 0) {
            (0, (_electronBuilderUtil || _load_electronBuilderUtil()).log)(`No native production dependencies`);
            return;
        }
        const platform = options.platform || process.platform;
        const arch = options.arch || process.arch;
        const additionalArgs = options.additionalArgs;
        (0, (_electronBuilderUtil || _load_electronBuilderUtil()).log)(`Rebuilding native production dependencies for ${platform}:${arch}`);
        ...
        if (isYarn) {
            ...
            yield (_bluebirdLst2 || _load_bluebirdLst2()).default.map(nativeDeps, function (dep) {
                (0, (_electronBuilderUtil || _load_electronBuilderUtil()).log)(`Rebuilding native dependency ${dep.name}`);
                return (0, (_electronBuilderUtil || _load_electronBuilderUtil()).spawn)(execPath, execArgs, {
                    cwd: dep.path,
                    env
                }).catch(function (error) {
                    if (dep.optional) {
                        (0, (_electronBuilderUtil || _load_electronBuilderUtil()).warn)(`Cannot build optional native dep ${dep.name}`);
                    } else {
                        throw error;
                    }
                });
    ...
    return function rebuild(_x5, _x6) {
...
function getGypEnv(frameworkInfo, platform, arch, buildFromSource) {
    ...
    return Object.assign({}, process.env, { npm_config_disturl: "https://atom.io/download/electron", npm_config_target: frameworkInfo.version, npm_config_runtime: "electron", npm_config_arch: arch, npm_config_target_arch: arch, npm_config_platform: platform, npm_config_build_from_source: buildFromSource, npm_config_devdir: getElectronGypCacheDir() });
}
function installDependencies(appDir, options) {
    const platform = options.platform || process.platform;
    const arch = options.arch || process.arch;
    const additionalArgs = options.additionalArgs;
    (0, (_electronBuilderUtil || _load_electronBuilderUtil()).log)(`Installing app dependencies for arch ${arch} to ${appDir}`);
    let execPath = process.env.npm_execpath || process.env.NPM_CLI_JS;
