...
class Packager {
    ...
    constructor(options, cancellationToken) {
        ...
        try {
            (0, (_electronBuilderUtil || _load_electronBuilderUtil()).log)("electron-builder " + "0.0.0-semantic-release");
        } catch (e) {
    ...
    build() {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            if ((_electronBuilderUtil || _load_electronBuilderUtil()).debug.enabled) {
                (0, (_electronBuilderUtil || _load_electronBuilderUtil()).debug)(`Effective config:\n${(0, (_jsYaml || _load_jsYaml()).safeDump)(JSON.parse((0, (_electronBuilderUtil || _load_electronBuilderUtil()).safeStringifyJson)(config)))}`);
            }
            yield (0, (_config || _load_config()).validateConfig)(config);
            _this._config = config;
            _this.appDir = yield (0, (_config || _load_config()).computeDefaultAppDirectory)(projectDir, (0, (_electronBuilderUtil || _load_electronBuilderUtil()).use)(config.directories, function (it) {
                return it.app;
            }));
            ...
            return {
                outDir,
                platformToTargets: yield (0, (_promise || _load_promise()).executeFinally)(_this.doBuild(outDir), function () {
                    return _this.tempDirManager.cleanup();
                })
            };
        })();
    }
    ...
    doBuild(outDir) {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            const taskManager = new (_asyncTaskManager || _load_asyncTaskManager()).AsyncTaskManager(_this3.cancellationToken);
    ...
    installAppDependencies(platform, arch) {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            if (config.nodeGypRebuild === true) {
                (0, (_electronBuilderUtil || _load_electronBuilderUtil()).log)(`Executing node-gyp rebuild for arch ${(_electronBuilderUtil || _load_electronBuilderUtil()).Arch[arch]}`);
                yield (0, (_electronBuilderUtil || _load_electronBuilderUtil()).exec)(process.platform === "win32" ? "node-gyp.cmd" : "node-gyp", ["rebuild"], {
                    env: (0, (_yarn || _load_yarn()).getGypEnv)(frameworkInfo, platform.nodeName, (_electronBuilderUtil || _load_electronBuilderUtil()).Arch[arch], true)
                });
            }
            if (config.npmRebuild === false) {
                (0, (_electronBuilderUtil || _load_electronBuilderUtil()).log)("Skip app dependencies rebuild because npmRebuild is set to false");
                return;
            }
            ...
            if (beforeBuild != null) {
                const performDependenciesInstallOrRebuild = yield beforeBuild({
                    ...
                    platform,
                    arch: (_electronBuilderUtil || _load_electronBuilderUtil()).Arch[arch]
                });
            ...
            if (config.buildDependenciesFromSource === true && platform.nodeName !== process.platform) {
                (0, (_electronBuilderUtil || _load_electronBuilderUtil()).log)("Skip app dependencies rebuild because platform is different and buildDependenciesFromSource is set to true");
            } else {
