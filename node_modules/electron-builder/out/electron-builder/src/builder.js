...
let build = exports.build = (() => {
    var _ref = (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* (rawOptions) {
        ...
        process.on("SIGINT", function () {
            (0, (_electronBuilderUtil || _load_electronBuilderUtil()).warn)("Cancelled by SIGINT");
    ...
    function processTargets(platform, types) {
        function commonArch(currentIfNotSpecified) {
            if (platform === (_core || _load_core()).Platform.MAC) {
                return args.x64 || currentIfNotSpecified ? [(_electronBuilderUtil || _load_electronBuilderUtil()).Arch.x64] : [];
            }
            const result = Array();
            if (args.x64) {
                result.push((_electronBuilderUtil || _load_electronBuilderUtil()).Arch.x64);
            }
            if (args.armv7l) {
                result.push((_electronBuilderUtil || _load_electronBuilderUtil()).Arch.armv7l);
            }
            if (args.ia32) {
                result.push((_electronBuilderUtil || _load_electronBuilderUtil()).Arch.ia32);
            }
            return result.length === 0 && currentIfNotSpecified ? [(0, (_electronBuilderUtil || _load_electronBuilderUtil()).archFromString)(process.arch)] : result;
        }
        ...
        for (const type of types) {
            ...
            if (suffixPos > 0) {
                (0, (_electronBuilderUtil || _load_electronBuilderUtil()).addValue)(archToType, (0, (_electronBuilderUtil || _load_electronBuilderUtil()).archFromString)(type.substring(suffixPos + 1)), type.substring(0, suffixPos));
            } else {
...
function createTargets(platforms, type, arch) {
    ...
    for (const platform of platforms) {
        const archs = platform === (_core || _load_core()).Platform.MAC ? [(_electronBuilderUtil || _load_electronBuilderUtil()).Arch.x64] : arch === "all" ? [(_electronBuilderUtil || _load_electronBuilderUtil()).Arch.x64, (_electronBuilderUtil || _load_electronBuilderUtil()).Arch.ia32] : [(0, (_electronBuilderUtil || _load_electronBuilderUtil()).archFromString)(arch == null ? process.arch : arch)];
