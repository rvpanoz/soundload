...
function computeArchToTargetNamesMap(raw, options, platform) {
    ...
    const defaultArchs = raw.size === 0 ? [platform === (_core || _load_core()).Platform.MAC ? "x64" : process.arch] : Array.from(raw.keys()).map(it => (_electronBuilderUtil || _load_electronBuilderUtil()).Arch[it]);
    ...
    for (const target of (0, (_electronBuilderUtil || _load_electronBuilderUtil()).asArray)(options.target).map(it => typeof it === "string" ? { target: it } : it)) {
        ...
        for (const arch of archs == null ? defaultArchs : (0, (_electronBuilderUtil || _load_electronBuilderUtil()).asArray)(archs)) {
            (0, (_electronBuilderUtil || _load_electronBuilderUtil()).addValue)(result, (0, (_electronBuilderUtil || _load_electronBuilderUtil()).archFromString)(arch), name);
        }
    }
    if (result.size === 0) {
        for (const arch of defaultArchs) {
            result.set((0, (_electronBuilderUtil || _load_electronBuilderUtil()).archFromString)(arch), []);
        }
    }
    return result;
}
...
function normalizeTargets(targets, defaultTarget) {
    ...
    for (const t of targets) {
        ...
        if (name === (_core || _load_core()).DEFAULT_TARGET) {
...
function createCommonTarget(target, outDir, packager) {
    ...
    } else if (target === (_core || _load_core()).DIR_TARGET) {
        return new NoOpTarget((_core || _load_core()).DIR_TARGET);
    } else {
        throw new Error(`Unknown target: ${target}`);
    }
}
class NoOpTarget extends (_core || _load_core()).Target {
    constructor(name) {
        super(name);
        this.options = null;
    }
