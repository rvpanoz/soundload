...
function _load_httpExecutor() {
    return _httpExecutor = require("../../electron-builder-http/src/httpExecutor");
}
...
class GitHubPublisher extends (_publisher || _load_publisher()).HttpPublisher {
    constructor(context, info, version) {
        ...
        if ((0, (_electronBuilderUtil || _load_electronBuilderUtil()).isEmptyOrSpaces)(token)) {
            ...
            token = token.trim();
            if (!(0, (_electronBuilderUtil || _load_electronBuilderUtil()).isTokenCharValid)(token)) {
            ...
            for (const release of releases) {
                if (release.tag_name === _this.tag || release.tag_name === _this.version) {
                    ...
                    if (_this.options.draft == null || _this.options.draft) {
                        (0, (_electronBuilderUtil || _load_electronBuilderUtil()).warn)(`Release with tag ${_this.tag} already exists`);
                        return null;
                    }
                    ...
                    if (publishedAt != null && Date.now() - publishedAt.getMilliseconds() > 2 * 3600 * 1000) {
                        ...
                        (0, (_electronBuilderUtil || _load_electronBuilderUtil()).warn)(`Release with tag ${_this.tag} published at ${publishedAt.toString()}, more than 2 hours ago`);
                        return null;
                    }
                    return release;
                }
            }
            (0, (_electronBuilderUtil || _load_electronBuilderUtil()).log)(`Release with tag ${_this.tag} doesn't exist, creating one`);
    ...
    deleteRelease() {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
                ...
                try {
                    ...
                    if (e instanceof (_electronBuilderHttp || _load_electronBuilderHttp()).HttpError) {
                        if (e.response.statusCode === 404) {
                            (0, (_electronBuilderUtil || _load_electronBuilderUtil()).warn)(`Cannot delete release ${release.id} — doesn't exist`);
                            return;
            ...
            (0, (_electronBuilderUtil || _load_electronBuilderUtil()).warn)(`Cannot delete release ${release.id}`);
        })();
    }
    githubRequest(path, token) {
        ...
        return (0, (_httpExecutor || _load_httpExecutor()).parseJson)((_nodeHttpExecutor || _load_nodeHttpExecutor()).httpExecutor.request((0, (_electronBuilderHttp || _load_electronBuilderHttp()).configureRequestOptions)({
